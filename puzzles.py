...
class Puzzles:
    ...
    def day1_1(self, input_file):
        bunny_path = list(map(str, open(input_file).readline().replace(' ', '').split(',')))
        return bunny_jumper(bunny_path)

    def day1_2(self, input_file):
        bunny_path = list(map(str, open(input_file).readline().replace(' ', '').split(',')))
        return bunny_jumper(bunny_path, full_course=False)
